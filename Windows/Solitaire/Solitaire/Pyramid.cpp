...
void CPyramid::NewGame(void)
{
	...
	//리스트2에서 랜덤한 위치에 카드값을 빼와서 리스트1에 넣음(카드 셔플)
	for(i=52;i>0;i--)						//52번 반복
	{
		randnum = rand()%i+1;				//랜덤한 값을 받아와서
		pos = m_List2.GetHeadPosition();	//Head위치로 부터
		for(j=0;j<randnum;j++)				//랜덤한 위치까지 접근 후
		{
			card = m_List2.GetNext(pos);	//card값을 받아와서
		}
...
//마우스 클릭을 처리할 함수
void CPyramid::MouseClick(CPoint point)
{
...
void CPyramid::MouseMove(CPoint point)
{
}
...
//리스트1이 선택되었나 체크하는 함수
bool CPyramid::IsClickList1(CPoint point)
{
	CRgn rgn;
...
//리스트2가 선택되었나 체크하는 함수
bool CPyramid::IsClickList2(CPoint point)
{
	CRgn rgn;
...
void CPyramid::DrawDoubleBuffering(CDC *pDC)
{
...
void CPyramid::Serialize(CArchive& ar)  
{
...
void CPyramid::CardShuffle(CDC *pDC, CCard card)
{
	CBitmap bitmap1;											//리스트1의 비트맵을 저장할 변수
	// 배경 출력. 이미지 구하면 바꾸기
	CDC dcmem;
...
void CPyramid::Animation(CDC *pDC)
{
	int x, y;									// 카드가 뿌려질 좌표 
